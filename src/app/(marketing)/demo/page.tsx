...
export const metadata: Metadata = {
  title: "Book a Demo",
  description: "See SwiftDispatch live — dispatch board, technician portal, quote flow, and SMS communication in one 30-minute walkthrough.",
    ...
    <main>
      <section className="px-6 pb-16 pt-16 sm:pt-20">
        <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-start">
          <div>
            <span className="font-mono text-[11px] uppercase tracking-[0.06em] text-orange-600">Request a Demo</span>
            <h1 className="mt-4 max-w-2xl text-5xl font-semibold tracking-[-0.025em] text-slate-950 sm:text-6xl">
            ...
            <div className="mt-8 grid gap-3">
              {demoHighlights.map((item, index) => (
                <div className="flex items-start gap-4 rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]" key={item}>
                  <span className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-950 font-mono text-sm font-semibold text-white">
                    0{index + 1}
                  </span>
                  <div>
                    <p className="font-mono text-[10px] uppercase tracking-[0.06em] text-slate-400">Demo step</p>
          ...
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <div className="border-b border-slate-100 px-6 py-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <span className="font-mono text-[11px] uppercase tracking-[0.06em] text-teal-700">Demo Intake</span>
                  <h2 className="mt-0.5 text-xl font-semibold tracking-tight text-slate-950">Tell us about your operation</h2>
                </div>
                <span className="rounded border border-teal-200 bg-teal-50 px-2 py-0.5 font-mono text-[10.5px] uppercase tracking-[0.06em] text-teal-700">Step 1 of 3</span>
              </div>
      ...
      <section className="px-6 py-20">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
            {fitCards.map((card) => (
              <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]" key={card.title}>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
              <div className="flex items-center gap-3 text-teal-700">
                ...
                <p className="font-mono text-[11px] uppercase tracking-[0.06em]">Best fit</p>
              </div>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
              <div className="flex items-center gap-3 text-orange-600">
                ...
                <p className="font-mono text-[11px] uppercase tracking-[0.06em]">Probably not a fit</p>
              </div>
              <div className="mt-5 space-y-3">
                {notFit.map((item) => (
                  <div className="flex items-start gap-3" key={item}>
      ...
      <section className="px-6 pb-24">
        <div className="mx-auto max-w-7xl overflow-hidden rounded-2xl bg-[linear-gradient(135deg,#0b2235_0%,#102f47_58%,#081b2a_100%)] px-8 py-12 shadow-[var(--shadow-lg)] lg:px-12">
          <div className="grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            ...
            <div className="grid gap-3">
              {[
                { icon: ClipboardList, label: "Dispatch review", step: nextSteps[0] },
                { icon: MessageSquareText, label: "Workflow walkthrough", step: nextSteps[1] },
                { icon: CalendarDays, label: "Fit decision", step: nextSteps[2] },
              ].map(({ icon: Icon, label, step }) => (
                <div className="rounded-xl bg-white/5 p-4" key={label}>
                  <div className="flex items-center gap-3 text-teal-300">
                    <Icon className="h-4 w-4" />
                    <span className="font-mono text-[10.5px] uppercase tracking-[0.06em]">{label}</span>
                  </div>
